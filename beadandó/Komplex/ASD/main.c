...
int main() {
    ...
    int futarokfutarok_pizza[10], napok[30];
    /**********************************************/
    char leg_betu;
    int leg_db = 0, napok_db[31], honap_pizza_db[6];
    for (i = 0; l[i].futar > 0; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            futarokfutarok_pizza[l[i].futar] += l[i].pizza[j];
            napok[l[i].nap] += l[i].pizza[j];
            honap_pizza_db[j] += l[i].pizza[j];
        }
    }
    /*****************************************************/
    printf("\nfutarok\n");
    for (i = 0; i < 10; i++) printf("%d : %d\n", i, futarokfutarok_pizza[i]);

    leg_db = 0;
    int leg_futar = 0;
    for (i = 0; i < 10; i++)
        if (futarokfutarok_pizza[i] > leg_db) {
            leg_futar = i;
            leg_db = futarokfutarok_pizza[i];
        }

    leg_db = 99;
    int legk_nap = 0;
    for (i = 1; i < 30; i++) {
        if (napok[i] > 0 && leg_db > napok[i]) {
            legk_nap = i;
            leg_db = napok[i];
        }
    }
    ...
    char legn_pizza = 0;
    for (i = 0; i < 6; i++) {
        if (leg_db < honap_pizza_db[i]) {
            legn_pizza = 'A' + i;
            leg_db = honap_pizza_db[i];
        }
    }

    printf("\n\nhonap pizzai\n");
    for (i = 0; i < 6; i++) printf("%c :: %d\n", 'A' + i, honap_pizza_db[i]);
